functions{
    ...
    real likeli(vector theta, int N_S, int L_S, matrix seqs, int[] ct_0, int[] ct_1, int n){
      // DNA energies
        ...
        p_E_0 *= 1/norm;
        p_E_1 *= 1/norm;
        real L = n * I(p_E_0, p_E_1, 500) * log(2);
        return L;
        }
}
...
transformed parameters{
    vector[4*L_S] theta_g = gauge_energy(centering_matrix, theta, L_S);
...
model{
    ...
    theta ~ normal(0, 1); 
    target += likeli(theta_g, N_S, L_S, seqs, ct_0, ct_1, n);
